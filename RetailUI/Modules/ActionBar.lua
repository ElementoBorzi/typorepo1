...
function Module:UpdateMainActionBar()
    -- Принудительно обновляем главную панель действий
    for _, actionBar in pairs(self.actionBars) do
        if actionBar.ID == MAIN_ACTION_BAR_ID then
            ReplaceBlizzardActionBarFrame(actionBar)
        end
    end
end

function Module:UNIT_ENTERED_VEHICLE()
    self:UpdateMainActionBar()
end
...
function Module:UPDATE_BONUS_ACTIONBAR()
    self:UpdateMainActionBar()
end

function Module:UPDATE_VEHICLE_ACTIONBAR()
    self:UpdateMainActionBar()
end
